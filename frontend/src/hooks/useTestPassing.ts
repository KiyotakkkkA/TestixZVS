...
export const useTestPassing = (testId: string | null, questions: TestQuestion[]) => {
  ...
  const setAnswerEvaluation = useCallback(
    (questionId: number, evaluation: { scorePercent: number; comment: string; userAnswerText: string }) => {
      if (!session) return;
  ...
  const finishTest = useCallback(async () => {
    ...
    const fullAnswerThreshold = (() => {
      const v = Number(process.env.REACT_APP_FULL_ANSWER_CORRECT_FROM_PERCENT ?? 70);
    ...
    const finalResult: TestResult = {
      ...
      settings,
      incorrectReview,
      fullAnswerReview: fullAnswerReview.length > 0 ? fullAnswerReview : undefined,
      answers,
    };
