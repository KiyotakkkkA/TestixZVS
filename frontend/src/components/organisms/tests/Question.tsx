...
export const Question: React.FC<QuestionProps> = ({
    ...
    const showHintButton =
        !isFullAnswer && Boolean(settings?.hintsEnabled) && !isChecked;
    const isCheckAfterAnswer = isFullAnswer
        ? true
        : Boolean(settings?.checkAfterAnswer);
    ...
    const activeEvaluation = useMemo(() => {
        ...
        const currentText = String(
            (userAnswer as string[] | undefined)?.[0] ?? "",
        );
    ...
    const runCheckIfNeeded = async (): Promise<boolean> => {
        ...
        try {
            ...
            const nextState: "correct" | "wrong" = ev.correct
                ? "correct"
                : "wrong";
            ...
            <div className="pt-4">
                ...
                {(() => {
                    const QuestionComponent =
                        resolveQuestionComponent(question);
