...
export const TestResultsPage = () => {
    ...
    const getScoreMessage = (percentage: number) => {
        ...
        if (percentage >= 70)
            return "Удовлетворительно! Подтяните слабые места!";
    ...
    return (
        <div className="w-full max-w-2xl space-y-8 mx-auto">
            <div className="bg-slate-50 rounded-lg shadow-xl p-8 text-center">
                <h1 className="text-4xl font-bold text-slate-800 mb-2">
                    Тест завершён!
                </h1>
                <p className="text-slate-600 mb-8">Вот ваши результаты</p>
                ...
                <div className="relative w-48 h-48 mx-auto mb-8">
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                        <div
                            className={`text-5xl font-bold bg-gradient-to-r ${getScoreColor(result.percentage)} bg-clip-text text-transparent`}
                        >
                            {result.percentage}%
                        </div>
                ...
                typeof passThreshold === "number" ? (
                    <div className="mt-4 inline-flex items-center justify-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-4 py-3">
                        <span
                            className={
                ...
                <div className="grid grid-cols-3 gap-4 mt-8 pt-8 border-t border-slate-200">
                    ...
                    <div className="text-center">
                        ...
                        <div className="text-sm text-slate-600 mt-1">Время</div>
                    </div>
                </div>
                ...
                {result.fullAnswerReview &&
                result.fullAnswerReview.length > 0 ? (
                    <div className="mt-8 pt-8 border-t border-slate-200 text-left">
                        <h2 className="text-xl font-bold text-slate-800">
                            Оценка ответов (полный ответ)
                        </h2>
                        <p className="text-sm text-slate-600 mt-1">
                            Комментарии модели по вашим ответам. Эти пункты
                            показываются всегда.
                        </p>
                        ...
                        <div className="mt-4 space-y-4">
                            {result.fullAnswerReview.map((item) => (
                                <div
                                    key={item.questionNumber}
                                    className="rounded-lg border border-indigo-200 bg-slate-50 p-4"
                                >
                                    ...
                                    <div className="mt-3 text-sm text-slate-700">
                                        ...
                                        <div className="mt-1 whitespace-pre-wrap rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                                            {item.userAnswerText || "—"}
                                        </div>
                                    </div>
            ...
            <button
                ...
                className="w-full py-4 bg-gradient-to-r from-indigo-500 to-indigo-600 text-slate-50 font-semibold rounded-lg hover:from-indigo-600 hover:to-indigo-700 transition-all shadow-lg"
            >
                Пройти тест снова
            </button>
        </div>
