    ...
    return (
        <div className="w-full">
            <div className="mx-auto flex w-full max-w-3xl flex-col space-y-4">
                <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                        <div className="w-full sm:max-w-xs">
                            <div className="mb-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Сортировка</div>
                            <Selector
                                value={sort}
                                options={sortOptions}
                                onChange={(value) => setSort(value as TestListSort)}
                            />
                        </div>
                    </div>
                ...
                {!isLoadingTests && listItems.map((test) => (
                    <TestListElementCard key={`${test.source}-${test.id}`} test={test} />
                ))}
            </div>
                ...
                <div className="p-2 space-y-4">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button
                            primary
                            onClick={async () => {
                                const result = await createBlankTest({ title: testName });
                                if (result?.testId) {
                                    addToast({
                                    'type': 'success',
                                    ...
                                    setTestName('');
                                    navigate('/workbench/test/' + result.testId);
                                } else {
                                    addToast({
                                        'type': 'danger',
