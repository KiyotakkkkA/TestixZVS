            ...
            <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-3">
                    ...
                    {terms.map((term, index) => (
                        <div key={index} className="flex items-center gap-2">
                            <div className="w-7 text-xs font-semibold text-indigo-500">
                ...
                <div className="space-y-3">
                    ...
                    {meanings.map((meaning, index) => (
                        <div key={index} className="flex items-center gap-2">
                            <div className="w-7 text-xs font-semibold text-emerald-500">
                                {index + 1}
                            </div>
                            <InputSmall
                                value={meaning}
            ...
            <div className="space-y-3">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    {meanings.map((meaning, index) => (
                        <div
                            key={`match-${index}`}
                            className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-100 px-3 py-2"
                        >
                            <div className="text-xs text-slate-500">
                                {index + 1}.
                            </div>
                            <div className="flex-1 truncate text-sm text-slate-600">
                                {meaning || `Определение ${index + 1}`}
                            </div>
                            <div className="w-36">
                                <Selector
                                    value={matches[index] ?? ""}
