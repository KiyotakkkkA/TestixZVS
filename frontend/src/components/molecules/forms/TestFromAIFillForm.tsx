...
const parseSelection = (raw: string, total: number): ParsedSelection => {
  ...
  if (!value) {
    return { indices: Array.from({ length: total }, (_, i) => i + 1), error: null };
  }
...
export const TestFromAIFillForm = ({ testId, onSuccess }: TestFromAIFillFormProps) => {
  ...
  const parsedSelection = useMemo(() => parseSelection(selection, total), [selection, total]);

  const selectedQuestions = useMemo(
    () => parsedSelection.indices.map((idx) => questions[idx - 1]).filter(Boolean),
  ...
  const handleSubmit = async (replace = false) => {
    ...
    try {
      ...
      toast.danger(e?.response?.data?.message || 'Не удалось импортировать вопросы');
    } finally {
    ...
    <div className="space-y-5 pb-2">
      ...
      {questions.length ? (
        <div className="space-y-4">
          <div className="space-y-2">
            <div className="text-sm font-semibold text-slate-700">Какие вопросы импортировать</div>
            <InputSmall
              ...
              className="py-2"
            />
            <div className="text-xs text-slate-500">Форматы: 3, 1,4,7 или 5:10. Индексация с 1.</div>
            {parsedSelection.error && (
              <div className="text-xs text-rose-600">{parsedSelection.error}</div>
            )}
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {selectedQuestions.map((item, index) => {
              ...
              const title = item?.question || item?.title || 'Без текста';
              const type = item?.type ? typeLabels[item.type] : 'Неизвестно';
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-1 text-sm font-semibold text-slate-800 line-clamp-3">{title}</div>
      ...
      <Modal
        ...
        title={<h3 className="text-lg font-semibold text-slate-800">Подтвердите замену</h3>}
        outsideClickClosing
      >
        <div className="space-y-4">
          <p className="text-sm text-slate-600">
            Все текущие вопросы будут удалены и заменены новыми. Продолжить?
          </p>
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            <Button secondary className="flex-1 p-2" onClick={() => setIsReplaceConfirmOpen(false)}>
              Отмена
            </Button>
            <Button
              danger
