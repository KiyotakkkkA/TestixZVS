...
export const mapApiQuestionToDraft = (
    question: TestQuestionResponse,
): QuestionDraft => {
    const options = question.options ?? {};

    const base = createQuestionDraft(
        question.id,
        question.type as QuestionDraftType,
    );
    const mapped = {
        ...base,
    ...
    if (
        mapped.type === "matching" &&
        mapped.matches.length < mapped.meanings.length
    ) {
        mapped.matches = [
            ...mapped.matches,
            ...Array(mapped.meanings.length - mapped.matches.length).fill(""),
        ];
    }
...
const serializePayload = (
    payload: TestQuestionPayload | null | undefined,
): string => {
    if (!payload) return "";
...
export const isDraftEmpty = (draft: QuestionDraft): boolean => {
    ...
    const hasMeanings = draft.meanings.some(
...
export const isDraftChanged = (draft: QuestionDraft): boolean => {
    ...
    const payloadChanged =
        ...
        serializePayload(draft.lastSavedPayload ?? null);
    const filesChanged =
        (draft.removedFileIds?.length ?? 0) > 0 ||
        (draft.media?.length ?? 0) > 0;

    return payloadChanged || filesChanged;
};

export const mapDraftToPayload = (
    draft: QuestionDraft,
): TestQuestionPayload => {
    switch (draft.type) {
        case "single":
