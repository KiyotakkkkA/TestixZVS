...
import { Button, Select } from "@kiyotakkkka/zvs-uikit-lib/ui";
...
function getVisiblePages(page: number, lastPage: number): (number | "dots")[] {
  if (lastPage <= 7) {
      ...
      <div className="grid grid-cols-[1fr_auto] items-center gap-2 sm:flex sm:flex-row sm:items-center sm:gap-3">
        {onPerPageChange && (
          <div className="flex min-w-0 items-center justify-between gap-2 rounded-md bg-main-900/35 text-sm text-main-300 sm:border-0 sm:bg-transparent sm:px-0 sm:py-0">
            <span className="truncate hidden sm:inline">На странице</span>
            <Select
              ...
              classNames={{
                ...
                option:
                  "px-2 py-1 text-sm transition hover:bg-main-700 data-[highlighted]:bg-main-700 data-[highlighted]:text-main-100 text-center",
              }}
              menuWidth={80}
